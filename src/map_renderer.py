...
def render_map(gdf_pse_group, coluna_selecionada):
    ...
    fig = px.choropleth_mapbox(
        ...
        center={
            "lat": -10.626485623243966,
            "lon": -37.072524582987405,
        },  # Centro do mapa (latitude e longitude)
        zoom=7,  # Nível de zoom do mapa
        opacity=0.7,  # Opacidade das áreas coloridas
        range_color=[
            0,
            zmax,
        ],  # Intervalo de cores do mapa (de 0 ao valor máximo da coluna)
        labels={
    ...
    fig.add_annotation(
        text=f"<em>Total: {total:,.0f}</em>".replace(
            ",", "."
        ),  # Texto da anotação com o total formatado
    ...
    fig.update_layout(
        title={
            "text": coluna_selecionada.replace(
                "Parti. de", "Participantes -"
            ),  # Texto do título (substitui parte do texto)
