...
def load_data():
    ...
    dim_municipio_chropleth = pd.read_csv(
        r"data/dimMunicipio.csv", dtype={"id_municipio_ibge": str}
    )
    ...
    atividades_temas_pse = pd.read_csv(
        ...
        dtype={
            "Ibge": str,
            "INEP (Escolas/Creche)": str,
        },  # Define colunas específicas como string
    ).iloc[
        :-2, 1:
    ]  # Remove as últimas duas linhas e a primeira coluna
