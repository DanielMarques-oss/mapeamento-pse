...
def transform_data(se_shp, dim_municipio_chropleth, atividades_pse2024, parti_pse):
    ...
    nova_colunas_parti = [
        f"Participantes: {col}" if col not in ["Uf", "Ibge", "Municipio"] else col
        for col in parti_pse.columns
    ]
    parti_pse.columns = nova_colunas_parti

    # Agrupando os dados
    grouped_atividades = (
        atividades_pse2024.groupby(["Ibge", "Municipio"])[
            atividades_pse2024.iloc[:, 4:-1].columns
        ]
        .sum()
        .reset_index()
    )
    grouped_parti = (
        parti_pse.groupby(["Ibge", "Municipio"])[parti_pse.iloc[:, 4:-1].columns]
        .sum()
        .reset_index()
    )
    ...
    col_name_part_semana = pse.columns[
        pse.columns.str.startswith("Participantes: Semana")
    ][0]
    ...
    pse_group = (
        pse.groupby(["Região de Saúde", "Ibge", "Municipio", "geometry"], dropna=False)[
            colunas_agg
        ]
        .sum()
        .reset_index()
